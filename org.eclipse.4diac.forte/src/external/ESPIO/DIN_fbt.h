...
class FORTE_DIN final : public CBasicFB {
  DECLARE_FIRMWARE_FB(FORTE_DIN)
  ...
  public:
    FORTE_DIN(CStringDictionary::TStringId paInstanceNameId, CResource *paSrcRes);
    ...
    void evt_INIT(const CIEC_UDINT &paCHNL, CIEC_BOOL &paQO) {
      var_CHNL = paCHNL;
      executeEvent(scmEventINITID, nullptr);
      paQO = var_QO;
    }

    void evt_REQ(const CIEC_UDINT &paCHNL, CIEC_BOOL &paQO) {
      var_CHNL = paCHNL;
      executeEvent(scmEventREQID, nullptr);
      paQO = var_QO;
    }

    void operator()(const CIEC_UDINT &paCHNL, CIEC_BOOL &paQO) {
      evt_INIT(paCHNL, paQO);
    }
};
